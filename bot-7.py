...
async def profilim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    daraja      = get_daraja(user.get("ball", 0))
    ball        = user.get("ball", 0)
    test_soni   = user.get("test_soni", 0)
    baj_kunlar  = len(user.get("bajarilgan_kunlar", []))
    yutuq_soni  = len(user.get("yutuqlar", []))
    referal_cnt = user.get("referal_count", 0)
    qoshilgan   = user.get("qoshilgan_sana", "—")
    chiziq      = "─" * 28
    ...
    text = (
        ...
        f"🎖 Daraja: {daraja}\n"
        f"⭐ Ball: {ball}\n"
        f"✅ Testlar: {test_soni} ta\n"
        f"📈 O'rtacha: {ortacha:.1f} ball\n"
        f"🔥 Streak: {streak} kun\n"
        f"📅 Bajarilgan: {baj_kunlar}/28 kun\n"
        f"🏆 Yutuqlar: {yutuq_soni} ta\n"
        f"👥 Referal: {referal_cnt} ta\n"
