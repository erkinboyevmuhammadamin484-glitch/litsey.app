...
# TEST
# ══════════════════════════════════════════════
async def _test_boshlash(update: Update, context: ContextTypes.DEFAULT_TYPE, fan: str, nom: str):
...
async def javob_qabul(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "test" not in context.user_data:
        await query.message.reply_text("❌ Test topilmadi. /start bosing.", reply_markup=main_keyboard())
        return
...
async def maqsadlar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["maqsad_mode"] = True
...
def main():
    ...
    # Bitta universal handler — ConversationHandler YO'Q
